	...
	<view class="u-page">
		<u-list>
			<u-list-item v-for="(item, index) in indexList" :key="index" >
				<uni-card :title="`upi账户${item.upiAccount}`" @click="changeData(item)">
					<view>
		...
		<uni-popup ref="popup" background-color="#fff">
			<uni-popup-dialog ref="inputClose" mode="input" :title="(isEdit?'修改':'绑定')+(accountType==1?'银行卡':'UPI')" value="对话框预置提示内容!"
				...
				<view class="popup-content">
					...
					<uni-easyinput style="margin-top: 10rpx;" v-model="dialogForm.accountName" :clearable=false
						...
						placeholderStyle="color: '#CCCCCC'"></uni-easyinput>
						<view>是否启用<u-switch v-model="dialogForm.accountStatus" activeValue='0' inactiveValue='1' size="50"></u-switch></view>
