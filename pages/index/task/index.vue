	...
	<view class="container">
		...
		<view class="notice">
			<image src="../../../static/icon_bottom_notice.png" style="width: 46rpx;margin: 0 17rpx 0 11rpx;"
				mode="widthFix"></image>
			<view style="flex-wrap: nowrap;overflow: hidden;">{{$t('立法流程要求获得流程和做法的许可。')}}
			</view>
		</view>
		<view class="content">
			<view class="tab" style="margin-top: 50rpx;">
				<view class="left" :class="{active:accountType==0}" @click="tapClick(0)">{{$t('代收')}}</view>
				<view class="right" :class="{active:accountType==1}" @click="tapClick(1)">{{$t('代付')}}</view>
			</view>
			...
			<view v-if="list.length>0">
				<view class="content-card" v-for="item in list">
					...
					<view class="line3" v-if="current==0">
						<view class="line3-title">{{$t('账户')}}：{{item.upiAccount}}</view>
